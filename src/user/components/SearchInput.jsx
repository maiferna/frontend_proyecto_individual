...
export const SearchInput = ({ ingredients, setIngredients, setRecipes }) => {
    ...
    const { data, loading, error, fetchData } = useFetch(`${urlBase}recipes/ingredients`, {
        method: 'POST'
    })
    ...
    return (
        <section className="container mt-4">
            ...
            <p className="text-center mb-3">Añade hasta 7 ingredientes</p>
            <form onSubmit={handleAdd} className="mb-3 d-flex">
                ...
                />
                <button type="submit" className="btn btn-green-border">+</button>
            </form>

            <div className="mb-3">
                {ingredients.map((name) => (
                    <div key={name} className="badge badge-green me-2 d-inline-flex align-items-center justify-content-between px-2 py-1">
                        {name}
                        <button
                            type="button"
